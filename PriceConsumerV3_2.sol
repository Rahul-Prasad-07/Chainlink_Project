...
contract PriceConsumerV3 {
    ...
    function getLatestPrice1() public  returns (int)
    {
        (, int price,,,) = priceFeed1.latestRoundData();
        return BTC = (int)(price);
    } // end function 1

    function getLatestPrice2() public returns (int) {
        (, int price,,,) = priceFeed2.latestRoundData();
        return ETH =  int(price * 1e10);
        // price = ETH;
    } // end function 2

    function getLatestPrice3() public  returns (int) {
        (, int price,,,) = priceFeed3.latestRoundData();
        return DAI = (int)(price);
        //price = DAI;
    } // end function 3
    ...
    function average (int BTC, int ETH, int DAI) public pure returns (int result)  {
